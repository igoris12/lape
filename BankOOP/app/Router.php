...
public function roter() {
    ...
    if ('POST' == $_SERVER['REQUEST_METHOD'] &&  $route == 'addMoney' && isset($_GET['id'])) {
        require 'C:\xampp\htdocs\lape\BankOOP\views/add.php';
        die;
      
    }
    // elseif ('GET' == $_SERVER['REQUEST_METHOD'] &&  $route == 'addMoney' && isset($_GET['id'])) {
    //     require 'C:\xampp\htdocs\lape\BankOOP\views/add.php';
    //     die;
    // }
    ...
    }elseif ('POST' == $_SERVER['REQUEST_METHOD'] &&  $route == 'add' && isset($_GET['id'])) {
    $bank = new BankControl;
    $data = $bank->showAll();
    $id = $_GET['id'];
    $bank->update($id,$data);
